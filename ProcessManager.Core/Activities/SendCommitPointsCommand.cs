...
{
    public class SendCommitPointsCommand :
        Activity<SampleState>
    {
        private readonly Uri _pointsServiceAddress;

        public SendCommitPointsCommand(Uri pointsServiceAddress)
        {
        ...
        public async Task Execute(BehaviorContext<SampleState> context, Behavior<SampleState> next)
        {
            ...
            {
                throw new Exception("Could not obtain the consumer context to send the CommitPoints command.");
            }
            ...
            await consumeContext.GetSendEndpoint(_pointsServiceAddress)
                .ContinueWith(
                    task =>
                        task.Result.Send(
                            new Commands.CommitPoints(context.Instance.RedemptionId, context.Instance.PointsExchangeId)),
